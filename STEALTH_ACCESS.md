...
### 2. 密码学防伪造签名（Cryptographically Signed Cookie）
* 我们废弃了最初版本中简单的 `1` 硬编码验证。
* 采用了基于 **Web Crypto API**（无任何外部模块依赖，在 Vercel Edge Runtime 极速运行）生成的 **HMAC SHA-256 时间戳签名**。
* 鉴权 Cookie 的生成格式为 `时间戳:哈希签名`（`${timestamp}:${signature}`），其中哈希签名由 `${timestamp}:${ACCESS_TOKEN}` 密文生成。
* 中间件在接收请求时会动态解析并重新比对签名。若无密钥，外界无法伪造此 Cookie，保障了绝对的访问安全性。
...
**💡 体验亮点**：
* 中间件校验密钥成功后，会在浏览器写入一个为期 1 年的安全加密 Cookie（`umami_access_token=时间戳:哈希签名`）。
